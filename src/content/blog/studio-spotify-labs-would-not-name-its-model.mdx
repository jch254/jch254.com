...
I had asked it to read Studio's own session data and interpret it, and it declined, saying file access to my home directory was outside its sandbox. That is not accurate. Spotify's [how it works page](https://labs.spotify.com/studio/how-it-works) says Studio can access files you point it to and is blocked from sensitive locations by default, which is a different claim entirely, and it matches what I saw on disk: the credential directory refused me and nothing else did. It did not matter anyway. When I stopped asking it to interpret and started asking it to execute a command and paste the raw output, it did, every time, including against its own installation.
...
I ran this past Spotify before publishing. I sent their security team a heads-up describing what I had found and what the post would say, then the full draft as it would appear, with a publication date and an open offer to correct anything factual: technical detail, product naming, anything I had misread from the bundle or the logs.

They were quick and courteous throughout. They thanked me for reaching out and said they appreciated the thoughtful approach. On substance, Spotify's policy is that they do not confirm, deny or otherwise comment on the specifics of their product architecture or implementation, so their review of the draft should not be read as an endorsement, verification or approval of anything in it. They had no objection to me publishing on my timeline, and the decision to publish and the content stayed mine.

That is a reasonable answer from their side, and it changes nothing above. Every claim in this post rests on primary sources from my own machine, the gateway source in the bundle and the log on my own disk, and it stands or falls on those.
